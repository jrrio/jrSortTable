...
(function () {
  ...
  jrSortTables.sort_functions = (function () {
    var aa, bb, tmpa, tmpb, date1, date2,
        re = /^\D+|\W+$/g; // for removing currency and % symbols
    return {
      alphaNumeric: function (a, b) {
        // Code by Joao Rodrigues.
      ...
      sortNumberJS: function (a, b) {
        // Firstly remove the thousands separator, currency and % symbols
      ...
      sortNumber_nonJS: function (a, b) {
        /** JavaScript expects a dot as the decimal separator,
         *  but some countries use a comma instead.
         *  e.g. 23,478.96 in English/JS and 23.478,96 in Portuguese
         */
        // Firstly remove the thousands separator, currency and % symbols
        aa = a[0].replace(re, '').replace(/\./g, '');
        bb = b[0].replace(re, '').replace(/\./g, '');
   ...
   *  Define getElementText method at load-time because it can use
   *  innerText or textContent depending on the browser running this code,
   *  and we need to speed up next function calls while sorting.
   */
  jrSortTables.getElementText = (function () {
    var parseEl = document.createElement('div');
    if (typeof parseEl.textContent != 'undefined') {
      // DOM Level 3 -> IE9 and W3C standards compliant browsers.
      return function (el) {
        return (el.textContent).replace(/^\s+|\s+$/g, '');
      };
    } else if (typeof parseEl.innerText != 'undefined') {
      return function (el) {
...
window.onload = jrSortTables.setup;
